...
test('has main UI', async ({ page }) => {
  await expect(page.locator('text=/rss агрегатор/i')).toBeVisible();
  await expect(page.locator('text=/Начните читать RSS сегодня. Это легко, это красиво./i')).toBeVisible();
...
test('open feed', async ({ page }) => {
  ...
  await expect(page.locator('text=/RSS успешно загружен/i')).toBeVisible();
});
